#!/usr/bin/env python3
"""
DAY5 batch 2b/2c: minimal, dependency-free process-held local mutual
exclusion for this project's single Day 5 mutation target (one kind
...
DAY5 batch 2c: replaces batch 2b's bare `DAY5_LOCK_HELD=1` boolean
trust mechanism, which had two reproduced defects:

  1. An UNRELATED process that merely had `DAY5_LOCK_HELD=1` set in
...
for `X`, and that recipe's own nested `day5_lock.py run` invocation).

So: the ACTUAL acquirer opens the lock file, `flock()`s it, and runs
its wrapped command with that exact fd passed through
(`pass_fds=(fd,)`) and its number exported as `DAY5_LOCK_FD`. A
process that sees `DAY5_LOCK_FD` set must never trust it at face
...
Recursive `make` ownership, concretely: `make day5-check`'s recipe is
`$(DAY5_LOCK) sh -c '$(MAKE) tool-check && ... && $(MAKE)
storage-bootstrap && ...'`. The `day5_lock.py run` invocation there is
the real acquirer; its direct child is the `sh -c` shell, which
inherits the fd and `DAY5_LOCK_FD`. That shell's own `make
storage-bootstrap` inherits both unchanged (make/bash do not strip
them), and `storage-bootstrap`'s own recipe line
(`$(DAY5_LOCK) python3 scripts/storage_bootstrap.py`) spawns yet
another `day5_lock.py run` - which sees `DAY5_LOCK_FD` already set,
...
FD_ENV = "DAY5_LOCK_FD"
LOCK_PATH_ENV = "DAY5_LOCK_PATH"
DEFAULT_LOCK_PATH = "/tmp/maops-day5-mutation.lock"
...
def _verify_inherited_fd() -> int | None:
    ...
    `DAY5_LOCK_FD` is unset, malformed, stale, or does not actually
...
def run(argv: list[str]) -> int:
    ...
    under the Day 5 mutation lock.

    If `DAY5_LOCK_FD` verifiably refers to an already-held instance of
    ...
    Documented boundary (DAY5 batch 2c specialist review): the
    ...
    try:
        try:
            ...
            print(
                f"FAIL: could not acquire the Day 5 mutation lock at {path!r} - "
                "another make/script invocation against this project's Day 5 target is already running "
...
def main() -> int:
    if len(sys.argv) < 3 or sys.argv[1] != "run" or sys.argv[2] != "--":
        print("usage: day5_lock.py run -- <command> [args...]", file=sys.stderr)
        return 2
