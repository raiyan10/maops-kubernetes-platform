#!/usr/bin/env python3
"""
Fail-closed Day 6 context/topology/node-version verification.

DAY6: this shared checker previously printed a stale "Day 4 context"
label while actually validating whichever day's context `kube.py`'s
constants currently pointed at (correctly Day 5, as of that release -
disclosed as a cosmetic, Low-severity wording issue in
`docs/roadmap.md`'s Day 5 section, deliberately left uncorrected until
"the shared checker is next advanced for Day 6"). This is that
correction: the module docstring/print now say Day 6, matching what the
live `kube.CONTEXT`/`kube.CLUSTER_NAME` constants (updated for Day 6 in
`scripts/kube.py`) actually verify.

DAY3-INT-M2: runs before any namespace apply, Secret bootstrap, image
load, or deploy step in `make day6-check` to prove, against the LIVE
cluster (never assumed from constants alone):

  - kubectl is actually talking to the isolated `kind-maops-k8s-day6`
    ...
    "maops-k8s-day6-staging-control-plane", which a bare `startswith()`
