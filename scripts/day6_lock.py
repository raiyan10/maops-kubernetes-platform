#!/usr/bin/env python3
"""
DAY6: minimal, dependency-free process-held local mutual exclusion for
this project's single Day 6 mutation target (one kind cluster, one
local operator) - an independent lock/lock-file from Day 4's/Day 5's
own (`scripts/day4_lock.py`/`scripts/day5_lock.py`, both preserved
unchanged as historical, frozen artifacts; never imported by this
module or by anything Day 6 runs). Not a general orchestration
framework - one `fcntl.flock()` on one fixed lock file, wrapping a
...
Day 6 adopts, unchanged in design, the verified-inherited-fd mechanism
Day 5's own lock arrived at after two remediation batches (2b/2c) found
and fixed two real defects in an earlier bare-boolean-flag trust
mechanism - see `scripts/day5_lock.py`'s own module docstring for that
full history. Day 6 never goes through that earlier, weaker design
itself; it starts directly from the already-hardened verified-fd
approach, reproduced here (with independent `DAY6_*` env vars and lock
path, never sharing state with Day 4's/Day 5's lock) because Day 6 is
its own separate mutation target requiring its own separate lock, not
because a defect was found in Day 6's own history. The two defects that
motivated this design, for context:

  1. An UNRELATED process that merely had a `..._LOCK_HELD=1`-style
     boolean set in its own environment (for any reason) would bypass
     acquisition entirely, even while a real, unrelated holder was
     active - a flag has no way to prove it was actually inherited FROM
     that holder.
...
for `X`, and that recipe's own nested `day6_lock.py run` invocation).

So: the ACTUAL acquirer opens the lock file, `flock()`s it, and runs
its wrapped command with that exact fd passed through
(`pass_fds=(fd,)`) and its number exported as `DAY6_LOCK_FD`. A
process that sees `DAY6_LOCK_FD` set must never trust it at face
...
Recursive `make` ownership, concretely: `make day6-check`'s recipe is
`$(DAY6_LOCK) sh -c '$(MAKE) tool-check && ... && $(MAKE)
storage-bootstrap && ...'`. The `day6_lock.py run` invocation there is
the real acquirer; its direct child is the `sh -c` shell, which
inherits the fd and `DAY6_LOCK_FD`. That shell's own `make
storage-bootstrap` inherits both unchanged (make/bash do not strip
them), and `storage-bootstrap`'s own recipe line
(`$(DAY6_LOCK) python3 scripts/storage_bootstrap.py`) spawns yet
another `day6_lock.py run` - which sees `DAY6_LOCK_FD` already set,
...
FD_ENV = "DAY6_LOCK_FD"
LOCK_PATH_ENV = "DAY6_LOCK_PATH"
DEFAULT_LOCK_PATH = "/tmp/maops-day6-mutation.lock"
...
def _verify_inherited_fd() -> int | None:
    ...
    `DAY6_LOCK_FD` is unset, malformed, stale, or does not actually
...
def run(argv: list[str]) -> int:
    ...
    under the Day 6 mutation lock.

    If `DAY6_LOCK_FD` verifiably refers to an already-held instance of
    ...
    Documented boundary (carried forward from Day 5's own batch 2c
    ...
    try:
        try:
            ...
            print(
                f"FAIL: could not acquire the Day 6 mutation lock at {path!r} - "
                "another make/script invocation against this project's Day 6 target is already running "
...
def main() -> int:
    if len(sys.argv) < 3 or sys.argv[1] != "run" or sys.argv[2] != "--":
        print("usage: day6_lock.py run -- <command> [args...]", file=sys.stderr)
        return 2
