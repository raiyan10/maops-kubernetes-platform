...
once per `make day4-check` invocation (see Makefile's DAY4_RUN_ID /
DAY4_SUITE_BASELINE_PATH) and passes through to child `$(MAKE)`
...
without DAY4_RUN_ID/DAY4_SUITE_BASELINE_PATH set in its environment -
i.e. `make state-check` or `make final-state-check` run on its own,
outside `make day4-check` - never claims suite-baseline capture or
...
DAY4_RUN_ID/DAY4_SUITE_BASELINE_PATH env vars will always disagree on
...
overlapping `make day4-check` invocations mutating the same live
cluster at once - a genuinely FRESH run computes its own new
`uuid4().hex`-derived path each time (see the Makefile's `DAY4_RUN_ID`),
...
written to twice (e.g. `DAY4_RUN_ID`/`DAY4_SUITE_BASELINE_PATH`
...
independent, concurrent invocation is `scripts/day4_lock.py`'s job
(DAY4 batch 2b, Part D), a wholly separate, process-held local lock -
not this per-run baseline file. Cleaning up a stale leftover baseline
file (from a run that crashed before final-state-check ever ran) is a
...
RUN_ID_ENV = "DAY4_RUN_ID"
PATH_ENV = "DAY4_SUITE_BASELINE_PATH"
