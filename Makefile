...
# Overridable kubeconfig path (DAY5) - every kubectl invocation below,
...
# DAY5 batch 2b/2c: a minimal process-held local mutual-exclusion lock
# (scripts/day5_lock.py) scoped to this project's single Day 5 target -
# closes the gap where a unique per-run baseline artifact (DAY5_RUN_ID/
# DAY5_SUITE_BASELINE_PATH above) does NOT prevent two independent,
...
# day5-check sequence as one lock acquisition. A child `$(MAKE) X`
# recipe line invoked recursively from an already-locked ancestor
# inherits the ancestor's actual locked file descriptor (via
# DAY5_LOCK_FD, verified - never a bare trusted flag) and skips
# re-acquiring; see day5_lock.py's own module docstring for the full
...
# killed). Deliberately independent of the DAY5_RUN_ID/baseline
# mechanism above - neither reads nor writes the other's file.
DAY5_LOCK := python3 scripts/day5_lock.py run --

# DAY5: run-specific suite-level `/state` baseline identity (see
...
# top-level `make day5-check` invocation generates ONE run ID/path and
...
# outside `day5-check`, with nothing already exported) instead
...
DAY5_RUN_ID := $(if $(DAY5_RUN_ID),$(DAY5_RUN_ID),$(shell python3 -c "import uuid; print(uuid.uuid4().hex)"))
DAY5_SUITE_BASELINE_PATH := $(if $(DAY5_SUITE_BASELINE_PATH),$(DAY5_SUITE_BASELINE_PATH),/tmp/maops-day5-suite-baseline-$(DAY5_RUN_ID).json)
export DAY5_RUN_ID
export DAY5_SUITE_BASELINE_PATH
...
# DAY5: local image-build/export settings needed for THIS local
...
.PHONY: help tool-check test version-check manifest-render manifest-check \
        ...
        rbac-check networkpolicy-check \
        final-state-check controller-check day5-check

help: ## Show this help
	@echo "MAOps Kubernetes Platform - Day 5 - available targets:"
	@# DAY5: the character class below now includes 0-9 - it previously
	@# excluded digits entirely, which silently dropped every dayN-check
	@# target (day2-check through day4-check, and this day's day5-check)
	@# from this listing without ever failing loudly.
...
manifest-render: ## Render the Day 5 Kustomize base with kubectl kustomize (pure local render, no cluster contact)
...
cluster-create: ## Create the scoped kind cluster (idempotent, never recreated if present) - maops-k8s-day5 only, 1 control-plane + 2 workers
	$(DAY5_LOCK) sh -c '\
		if kind get clusters 2>/dev/null | grep -qx "$(CLUSTER_NAME)"; then \
			echo "kind cluster $(CLUSTER_NAME) already exists, skipping create"; \
		else \
			kind create cluster --name $(CLUSTER_NAME) --config kind/cluster-day5.yaml --kubeconfig $(KUBECONFIG_PATH); \
		fi; \
		kubectl --kubeconfig $(KUBECONFIG_PATH) --context $(KCONTEXT) get nodes \
	'

cluster-delete: ## Delete ONLY the maops-k8s-day5 kind cluster
	$(DAY5_LOCK) kind delete cluster --name $(CLUSTER_NAME) --kubeconfig $(KUBECONFIG_PATH)

context-check: ## Fail closed unless kubectl is verified against the isolated Day 5 cluster at the pinned node version
	python3 scripts/context_check.py

cni-install: ## Install/upgrade Cilium 1.20.1 via Helm as the Day 5 CNI dataplane (idempotent `helm upgrade --install`; kube-proxy stays enabled - no kubeProxyReplacement - kindnet is disabled via kind/cluster-day5.yaml's networking.disableDefaultCNI so every node is NotReady/no pod network until this runs)
	$(DAY5_LOCK) sh -c '\
...
storage-bootstrap: ## Harden new local-path-provisioner backing directories (root:10001, mode 2770) before any application PVC is created (needs `make image-load` first - its scratch probe Pod reuses the already-loaded app image)
	$(DAY5_LOCK) python3 scripts/storage_bootstrap.py
...
storage-hardening-check: ## Prove the storage bootstrap actually changed access behavior, via a disposable scratch PVC (never the application's own PVC; also needs `make image-load` first)
	$(DAY5_LOCK) python3 scripts/storage_hardening_check.py

namespace-apply: ## Apply ONLY the two project Namespaces (maops-platform, maops-day5-validation) to the explicit Day 5 context (must precede secret-bootstrap)
	$(DAY5_LOCK) kubectl --kubeconfig $(KUBECONFIG_PATH) --context $(KCONTEXT) apply -f $(BASE)/namespace.yaml -f $(BASE)/day5-validation-namespace.yaml
...
secret-bootstrap: ## Create (or preserve) both runtime Secrets - maops-internal-auth and maops-state-auth - never committed, never printed
	$(DAY5_LOCK) python3 scripts/secret_bootstrap.py all
...
deploy: ## Apply the full Day 5 Kustomize base (both Namespaces, ConfigMaps, Deployments, StatefulSet, Services, PDBs, ServiceAccounts, Role/RoleBinding, NetworkPolicies) to the kind cluster
	$(DAY5_LOCK) kubectl --kubeconfig $(KUBECONFIG_PATH) --context $(KCONTEXT) apply -k $(BASE)
...
workload-refresh: ## Force maops-state/maops-app/maops-gateway to restart through their own controllers, in that dependency order, so every workload actually runs THIS run's freshly built/loaded local image rather than whichever workload's `deploy` diff happened to trigger a rollout on its own - an intentional local-image refresh with a brief single-replica maops-state outage, never a zero-downtime claim; never touches PVC/PV/Secrets
	$(DAY5_LOCK) python3 scripts/workload_refresh.py
...
dependency-check: ## Prove gateway liveness vs. dependency-aware readiness by scaling maops-app to 0 and back
	$(DAY5_LOCK) python3 scripts/dependency_check.py

scaling-check: ## Prove real scaling behavior (3 -> 4 -> 3) for gateway/app, with guaranteed restoration
	$(DAY5_LOCK) python3 scripts/scaling_check.py

rolling-update-check: ## Prove a real rolling update and `kubectl rollout undo` rollback for gateway/app
	$(DAY5_LOCK) python3 scripts/rollout_check.py

pdb-check: ## Prove real PodDisruptionBudget/Eviction-API behavior for gateway/app (state carries no PDB)
	$(DAY5_LOCK) python3 scripts/pdb_check.py

state-check: ## Prove maops-state StatefulSet/PVC/Service runtime identity, security, and storage binding (also captures the run-specific suite-level /state baseline used by final-state-check, when DAY5_RUN_ID/DAY5_SUITE_BASELINE_PATH are set - see scripts/suite_baseline.py)
	python3 scripts/state_check.py

rbac-check: ## Prove maops-diagnostics RBAC scope from a live probe Pod: real HTTPS calls to the API server using its own mounted token, proving allowed reads (Pods/Services/EndpointSlices) and denied access (Secrets, workload mutation, deletion, scaling, cross-namespace, cluster-wide) - creates/deletes one short-lived probe Pod under the maops-diagnostics ServiceAccount
	$(DAY5_LOCK) python3 scripts/rbac_check.py

networkpolicy-check: ## Prove default-deny + explicit-allow NetworkPolicy behavior via real in-cluster probe Pods: validation-client->gateway allowed, validation-client->app/state denied, gateway->app and app->state allowed, gateway->state denied, DNS resolution works - creates/deletes short-lived probe Pods
	$(DAY5_LOCK) python3 scripts/networkpolicy_check.py

persistence-check: ## Prove data survives maops-state-0 pod deletion/rescheduling (same PVC/PV, new Pod UID, unchanged marker)
	$(DAY5_LOCK) python3 scripts/persistence_check.py

retention-check: ## Prove PVC/PV retention and app/gateway degraded-but-live behavior across a state 1 -> 0 -> 1 cycle
	$(DAY5_LOCK) python3 scripts/retention_check.py

final-state-check: ## Independently prove the cluster is fully restored to its normal Day 5 baseline after all mutating experiments (a standalone run without DAY5_RUN_ID/DAY5_SUITE_BASELINE_PATH from a prior state-check in the SAME invocation cannot claim suite-baseline restoration - see scripts/suite_baseline.py)
	python3 scripts/final_state_check.py

controller-check: ## (Bonus, not part of day5-check) Prove Deployment controller reconciliation for maops-app
	$(DAY5_LOCK) python3 scripts/reconcile_check.py

day5-check: ## Authoritative Day 5 validation sequence - explicitly sequential via recipe lines, never a parallelizable prerequisite list, so `make -j` can never reorder a mutation ahead of its precondition. Acquired under ONE Day 5 mutation lock for its entire duration (scripts/day5_lock.py) - a second independent day5-check (or any other standalone mutating target) invoked concurrently fails immediately, before this sequence's own first mutating step, rather than racing it
	$(DAY5_LOCK) sh -c '\
		$(MAKE) tool-check && \
		$(MAKE) test && \
		$(MAKE) version-check && \
	...
	'
	@echo ""
	@echo "PASS: day5-check completed the full authoritative validation sequence"
