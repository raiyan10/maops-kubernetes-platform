"""
Tests for scripts/day6_lock.py - the minimal process-held local mutual-
exclusion lock scoped to this project's single Day 6 target, an
independent lock/lock-file from Day 4's/Day 5's own.
...
Day 6 adopts Day 5's own already-hardened verified-inherited-ownership
design directly (see `scripts/day6_lock.py`'s module docstring, and
`scripts/day5_lock.py`'s for the full batch 2b/2c history of the
bare-boolean-flag defects that design fixes): a shared, `pass_fds`-
inherited file descriptor (`DAY6_LOCK_FD`, checked via
`/proc/self/fd/<n>` + a non-blocking flock probe) rather than a
trusted boolean. `test_forged_lock_fd_without_valid_ownership_does_not_bypass_active_holder`
below proves a forged/stale claim of ownership must never bypass a
real, active holder - the property this design exists to guarantee.
"""
...
SCRIPT = str(Path(__file__).resolve().parent.parent / "scripts" / "day6_lock.py")
...
class RealLockContentionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.lock_path = os.path.join(self.tmpdir.name, "day6-mutation.lock")
        self.env = dict(os.environ)
        self.env["DAY6_LOCK_PATH"] = self.lock_path
        self.env.pop("DAY6_LOCK_FD", None)
    ...
    def test_forged_lock_fd_without_valid_ownership_does_not_bypass_active_holder(self):
        """Proves the property Day 5's own batch 2c hardening established
        (see scripts/day6_lock.py's module docstring): a forged
        `DAY6_LOCK_FD` (an arbitrary number, not an actually-inherited
        duplicate of the real holder's fd) must be REJECTED by
        verification and fall through to a real acquire attempt - which
        must then correctly fail while a genuine holder is active,
        exactly like any other independent invocation. Never bypassed by
        a bare boolean/claimed value."""
        ready = os.path.join(self.tmpdir.name, "holder-ready.marker")
        ...
        try:
            ...
            forged_env["DAY6_LOCK_FD"] = "99999"  # not a real, inherited fd in this process
            forged = self._run("sh", "-c", f"touch {forged_ran}", env=forged_env)
        finally:
            _reap(holder)

        self.assertNotEqual(forged.returncode, 0, "a forged DAY6_LOCK_FD must never bypass a real active holder")
    ...
    def test_independently_opened_fd_on_same_path_genuinely_contends_and_fails(self):
        ...
        # and claims it via DAY6_LOCK_FD before invoking day6_lock.py.
    ...
    def test_forged_lock_fd_when_lock_is_actually_free_just_acquires_normally(self):
        """A forged/stale `DAY6_LOCK_FD` when NOTHING actually holds the
    ...
    def test_wrapper_termination_while_child_still_active_keeps_exclusion(self):
        """The core property this design guarantees. Killing ONLY the wrapper
        ...
        try:
            ...
            os.kill(wrapper.pid, signal.SIGKILL)  # kill ONLY the day6_lock.py wrapper
    ...
    def test_lock_is_scoped_to_the_lock_path_not_global(self):
        ...
        other_env["DAY6_LOCK_PATH"] = os.path.join(self.tmpdir.name, "different.lock")
