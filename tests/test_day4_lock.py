"""
Tests for scripts/day4_lock.py - the minimal process-held local mutual-
exclusion lock scoped to this project's single Day 4 target (DAY4
batch 2b/2c).
...
DAY4 batch 2c replaced the bare `DAY4_LOCK_HELD=1` boolean trust
...
(`DAY4_LOCK_FD`, checked via `/proc/self/fd/<n>` + a non-blocking
flock probe - see day4_lock.py's own module docstring for the full
...
SCRIPT = str(Path(__file__).resolve().parent.parent / "scripts" / "day4_lock.py")
...
class RealLockContentionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env["DAY4_LOCK_PATH"] = self.lock_path
        self.env.pop("DAY4_LOCK_FD", None)
    ...
    def test_forged_lock_fd_without_valid_ownership_does_not_bypass_active_holder(self):
        """DAY4 batch 2c: replaces the batch-2b test that treated an
        unrelated process setting `DAY4_LOCK_HELD=1` as proof of
        successful recursive ownership. A forged `DAY4_LOCK_FD` (an
        ...
        try:
            ...
            forged_env["DAY4_LOCK_FD"] = "99999"  # not a real, inherited fd in this process
            forged = self._run("sh", "-c", f"touch {forged_ran}", env=forged_env)
        finally:
            _reap(holder)

        self.assertNotEqual(forged.returncode, 0, "a forged DAY4_LOCK_FD must never bypass a real active holder")
    ...
    def test_independently_opened_fd_on_same_path_genuinely_contends_and_fails(self):
        """DAY4 batch 2c (test-engineer review): `_verify_inherited_fd()`
        ...
        # and claims it via DAY4_LOCK_FD before invoking day4_lock.py.
    ...
    def test_forged_lock_fd_when_lock_is_actually_free_just_acquires_normally(self):
        """A forged/stale `DAY4_LOCK_FD` when NOTHING actually holds the
    ...
    def test_wrapper_termination_while_child_still_active_keeps_exclusion(self):
        """DAY4 batch 2c: the core Bug-B fix. Killing ONLY the wrapper
        ...
        try:
            ...
            os.kill(wrapper.pid, signal.SIGKILL)  # kill ONLY the day4_lock.py wrapper
    ...
    def test_lock_is_scoped_to_the_lock_path_not_global(self):
        ...
        other_env["DAY4_LOCK_PATH"] = os.path.join(self.tmpdir.name, "different.lock")
