...
1. **VERSION file.** Exact match to the day's target version (current
   released baseline: `0.5.0` for Day 5; use whatever
   ...
   confirm it independently passes - don't just eyeball the file.
2. **Git safety.** `git status` and `git log --oneline -5` should show
   ...
   days' tags (as of the current baseline: `v0.1.0` through `v0.5.0`)
   are expected to already exist and must never be moved or recreated.
3. **Authoritative validation actually ran.** Re-run
   `make dayN-check` for the current day (e.g. `make day5-check`) yourself
   ...
   `scripts/networkpolicy_check.py`, `scripts/cni_check.py`,
   ...
   `scripts/retention_check.py`, `scripts/final_state_check.py`, as
   applicable to the current day). A restoration failure surfaced by
   any mutating script (scaling, rolling update/rollback, PDB/Eviction,
   dependency-failure, persistence/retention) is more serious than an
...
7. **Scope boundaries.** Nothing from a later day leaked in early. As of
   the frozen `v0.5.0` baseline (Days 1-5 released), a Day 5 review
   should find dedicated ServiceAccounts, the `maops-diagnostics`
   `Role`/`RoleBinding`, seven NetworkPolicy objects, and Cilium as the
   enforcing CNI - these are now **required**, not scope violations. Nothing
   from Day 6/7 should appear yet: Helm-packaged application charts
   (Helm here is used only to install Cilium), GitHub Actions CI
   workflow files, a service mesh (mTLS/L7 policy), Ingress/Gateway API,
   `HorizontalPodAutoscaler`, or a Recreate/Blue-Green/Canary deployment
   strategy demonstration. Generic evergreen exclusions at any day:
   observability config, Terraform/Ansible/Argo CD files, and
   cloud-provisioning code. A *committed Secret object* is still always
   forbidden at any day - a runtime-bootstrapped Secret existing live in
   the cluster is correct, not a violation.
